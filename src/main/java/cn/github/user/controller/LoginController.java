...
public class LoginController {
    ...
    private Result<JSONObject> userInfo(SysUser sysUser, Result<JSONObject> result) {
        ...
        obj.put("rights", menuList(username));
        result.setResult(obj);
        result.success("登录成功");
        return result;
    }

    private List<MenuV> menuList(String username){
        //根据用户名获取用户角色的菜单
        List<MenuV> vList = sysUserService.getUserMenu(username);
        List<MenuSecV> secVList = sysUserService.getUserMenuSecond(username);
        for (MenuV menuV : vList) {
            List<MenuSecV> menuSecVS = new ArrayList<MenuSecV>();
            for (MenuSecV menuSecV : secVList) {
                if (menuV.getId().equals(menuSecV.getParentId()))
                    menuSecVS.add(menuSecV);
            }
            menuV.setChildren(menuSecVS);
        }
        return vList;
    }
